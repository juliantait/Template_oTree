...
(`python tests/<name>.py`), which is the suite's documented convention
(skills_claude/writing_tests.md). scripts/predeploy_check.py's
...
def task_page_submits():
    ...
    no-JS submit every suite must tolerate (conventions.md: an empty hidden
