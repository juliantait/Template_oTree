...
| **`scripts/tests/creed_lab_block_test.py`** — the CREED lab block at the end of `settings.py`, run as a **differential**: the file is executed in a child process twice under one byte-identical environment, as shipped and with everything from the start marker cut away, and the resolved values compared | that the block changes **no resolved setting** when the launcher's variables are absent AND under the exact environment `scripts/set_up_otree.bat` exports (read out of the bat file, not retyped) — `ROOMS` (including the `study` room's `welcome_page`), `ADMIN_USERNAME`, `ADMIN_PASSWORD`, `DEBUG`, `DATABASES` and `SECRET_KEY` all identical, so `set_up_otree.bat` and a plain `otree devserver` keep working unchanged; that both markers are intact (the launcher detects the block by **text match**) and the block is genuinely last, with nothing binding `ROOMS` after it and nothing mutating it; and that with `CREED_LABEL_FILE` set the room gains **exactly one** key and loses none | **it is NOT evidence that the launcher works** — the CREED Lab Launcher is not in this repository and is never run here, so what is tested is the block's behaviour under the variables the launcher is *documented* to export, not that it exports them. Nor that the seat labels in the file match the `participant_label=` in the real lab machines' desktop shortcuts — nothing here knows what those machines are. Nor anything against a **live Postgres or a live lab**: the `set_up_otree.bat` leg asserts the `DATABASES` dict that `settings.py` resolves and never opens a connection. Blind, too, to every page a participant or an operator then sees | after touching the block, `ROOMS`, either `ADMIN_USERNAME` line, or `scripts/set_up_otree.bat` |
...
| **`scripts/tests/dashboard_embed_height_test.py`** — real headless Chromium at three viewport heights, reading the actual iframe style out of `outro/admin_report.html` | that the embedded dashboard **fills the viewport** (100dvh, border-box, so it fits exactly), that the page is taller than the viewport so oTree's chrome **can be scrolled out of the top**, and that `display:block` leaves **no descender "leftover row"** below the iframe | how the dashboard's own content looks inside the iframe; oTree's real chrome height (reproduced, not measured) | after touching the iframe in `outro/admin_report.html` |
...
| **`scripts/tests/health_test.py`** — in-process, production + `AUTH_LEVEL=STUDY`: the shared route installer (`otree_routes.py`) and the `/health` route over real requests | that BOTH extra routes install from one implementation, idempotently, quiet when `otree.urls` is not importable and loud on drift, with a route under an undeclared name or on an occupied path refused; that the verdict flips 503→200→503 with the room binding **in one process**; that an unstamped build and a dirty pre-launch checklist both ride a **200**; that the open body carries the checklist NAMES and **no completion-code value**, with its key set asserted CLOSED so nothing about participants can arrive there unnoticed; and that breaking each block in turn still yields a 503 verdict, never a 500 | whether a real platform's health check is wired to it (that is a service setting this repo deliberately does not ship); HEAD, which starlette 0.14.1's TestClient cannot read — measured against a real prodserver instead | after touching `health.py`, `otree_routes.py`, either install call in `outro/__init__.py`, or `settings.ROOMS` |
| **`scripts/tests/verify_deploy_test.py`** — runs the real `scripts/verify_deploy.py` **as a subprocess** against a programmable stub deployment; needs no oTree and no database | that the gate FAILS on the states a healthy server will not produce on demand: another commit, no stamp at all, a mismatched commit/number pair, a 404 at `/health`, a room the REST API says is unbound *even when `/health` claims otherwise*, and a room page that is an error page, too small, missing the study's markup or redirecting; that it issues only GETs, requests nothing that consumes a participant slot, and never prints a participant label; and that frozen-config findings keep the audit's two severities plus the named `build_at_creation` exemption | whether a REAL deployment behaves as the stub does — that is the transcript in `_ai/build_provenance/phase2_test_output/verify_deploy_live.txt`, against a real prodserver | after touching `scripts/verify_deploy.py`, `health.py`'s payload shape, or `predeploy_check.audit_frozen_session_configs` |
