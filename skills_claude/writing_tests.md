...
more scripts in `tests/`, each runnable on its own (`python tests/<name>.py`) and
...
Two drivers, both already in `tests/`. Pick by what you need to reach.
...
`tests/render_check.py` drives headless Chromium over a real server, writes
...
LD_LIBRARY_PATH=<sysroot>/usr/lib/x86_64-linux-gnu python tests/render_check.py
```
...
`tests/example_quiz_content_test.py` is the model. Copy it, keep the shape —
