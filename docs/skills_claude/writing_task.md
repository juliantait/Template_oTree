...
**`scripts/tests/main_contract.py` — in the SAME change as `main/`:**
...
- `scripts/tests/example_quiz_content_test.py` — a MODEL, written against the
  placeholder quiz: copy it, keep the shape, rewrite the expectations
  (its own header says how).
- `scripts/tests/render_check.py` — after any task-template change, re-run; the card
...
- [ ] `scripts/tests/main_contract.py` updated in the same change as any page rename
