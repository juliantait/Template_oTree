...
- [`hosting_railway.md`](./hosting_railway.md) — how to put a study from this template onto Railway (the one managed deploy actually done, 2026-08-14): the deploy-repo export, project tokens, which operations need the GraphQL API rather than the CLI, the env vars, the crash rehearsal, study day. **Procedure only** — what a hosted deploy needs in general, what the boot guard does, and the caveats live in [`docs/hosting_a_prolific_study.md`](../docs/hosting_a_prolific_study.md), which is written for the researcher rather than the agent. Relevant to ONLINE studies only; a lab study needs none of it, and this template deliberately ships no deployment configuration.
