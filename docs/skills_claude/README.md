...
- [`hosting_a_prolific_study.md`](./hosting_a_prolific_study.md) — the end-to-end agent playbook for hosting AND running a study on Prolific: the order (deploy on Railway, wire the study to Prolific, create the Prolific study via the API), delegating the hosting detail to [`../hosting_railway.md`](../hosting_railway.md) and the study-side wiring to [`../running_on_prolific.md`](../running_on_prolific.md). Folds in the Prolific API procedure and its money/seat semantics: `reward` is in pence, the hourly minimum must be cleared by the base reward alone, places are completions-you-pay-for (returned and timed-out submissions reopen automatically), Prolific places must never exceed the oTree seat count, the two **no-API** human steps (Preview and Test-as-a-participant, the latter the only end-to-end check of the return-and-payment path), and the NOT VERIFIED publish caveat. Online studies only.
- [`hosting_railway.md`](../hosting_railway.md) — the hosting info doc and the one managed deploy actually done (2026-08-14): what a hosted deploy needs in general (container host, managed Postgres, `DATABASE_URL`, the boot guard, the untested caveats), plus the verified Railway PROCEDURE — the deploy-repo export, project tokens (and what a project token CANNOT do), which operations need the GraphQL API rather than the CLI, the attempt-then-verify rule, the deploy order for a schema-changing build, domains, the env vars, the crash rehearsal, study day. Relevant to ONLINE studies only; a lab study needs none of it, and this template deliberately ships no deployment configuration.
